# -*- coding: utf-8 -*-`
"""api.py - Contains the Game API for Hangman.
This includes endpoints to create users, and games. There are also endpoints
to get game information including the current state of the game, user scores
and ranks. Some of the endpoints get their logic (to_form, etc) from
the included models."""
...
from models import Game, User, Score, NewGameForm, MakeMoveForm, GameForm, \
    GameForms, HistoryForms, StringMessage, ScoreForms, RankForms
...
class HangmanApi(remote.Service):
    ...
    def make_move(self, request):
        ...
        if not request.guess.isalpha():
            raise endpoints.ForbiddenException(
                'Illegal action: String is not a string or letter.')
        ...
        if request.word_guess:
            ...
            if game.guess_history.find('"%s"' % request.guess.lower()) != -1:
                ...
                # Compares the two words and sets the state and message
                # accordingly
                if request.guess.lower() == game.target_word.lower():
                    ...
                    game.attempts_remaining -= 1
                    msg = 'Incorrect word!'
                    # If it's not the right word and there are no more
            ...
            if game.guess_history.find(': "' + request.guess.lower()) != -1:
                ...
                try:
                    ...
                    raise endpoints.BadRequestException(
                        'Only a single letter is allowed to be guessed!')
                ...
                if game.current_word.lower() == game.target_word.lower():
                    ...
                    game.end_game(True)
                    msg = 'You win!'
                # Sets the message and state if the user couldn't guess the
                # word within the attempt limit
                if game.attempts_remaining < 1:
                    game.end_game(False)
                    msg += ' Game over!'
        # For converting game history to a json, it will only add a comma it is
        # not the only guess
        if len(game.guess_history) < 1:
            game.guess_history += '{"Guess": "%s", "Result": "%s"}' % (
                request.guess.lower(), msg)
        else:
            game.guess_history += ', {"Guess": "%s", "Result": "%s"}' % (
                request.guess.lower(), msg)
        game.put()
    ...
    def get_high_scores(self, request):
        ...
        scores = Score.query(
            Score.won == True).order(-Score.guesses).fetch(limit=request.number_of_results)
        return ScoreForms(items=[score.to_form() for score in scores])
